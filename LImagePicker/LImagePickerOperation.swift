...
class LImagePickerOperation: Operation {
    
    typealias Completion = ((UIImage? , PHAsset?) -> ())
    
    fileprivate let isOriginal: Bool
    
    fileprivate let photographModel: LPhotographModel
    
    fileprivate let progress: PHAssetImageProgressHandler?
    
    fileprivate let completion: Completion
    
    init(photographModel: LPhotographModel, isOriginal: Bool, progress: PHAssetImageProgressHandler? = nil, completion: @escaping Completion) {
    ...
    override func start() {
        ...
        // 图片
        if isOriginal {
            LImagePickerManager.shared.getOriginalPhotoWithAsset(asset: photographModel.media, progressHandler: progress) { [weak self] (image, isDegraded) in
                if !isDegraded {
            ...
            LImagePickerManager.shared.getPhotoWithAsset(photographModel.media, size: CGSize(width: w, height: w * aspectRatio), resizeMode: .fast, progress: self.progress) { [weak self] (image, isDegraded) in
                if !isDegraded {
                    self?.completion(self?.scaleImage(image), nil)
                    self?.fetchFinish()
                }
