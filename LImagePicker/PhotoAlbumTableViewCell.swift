...
class PhotoAlbumTableViewCell: UITableViewCell {
    ...
    fileprivate func initView() {
        ...
        coverImage.leftAnchor.constraint(equalTo: contentView.leftAnchor, constant: 5).isActive = true
        coverImage.centerYAnchor.constraint(equalTo: contentView.centerYAnchor).isActive = true
    ...
    public func photoAsset(albumModel: PhotoAlbumModel) {
        nameLabel.text = albumModel.title
        guard let asset = albumModel.asset else { return }
        ImagePickerManager.shared.getPhotoWithAsset(asset, size: CGSize(width: 80.0 * LConstant.sizeScale, height: 80.0 * LConstant.sizeScale), progress: nil) { (image, isDegraded) in
